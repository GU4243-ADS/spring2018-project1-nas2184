...
mws_split <- mws_bigrams %>% 
  separate(bigram, c("word1", "word2"), sep = " ")

mws_filtered <- mws_split %>% 
  filter(!word1 %in% stop_words$word) %>% 
  filter(!word2 %in% stop_words$word) 
#understand what the stop_words$word; wouldn't this remove the "not"?

# group_by stop words? investigate the negation of words for analysis
